...
class CrmMarketingCampaign(models.Model):
    ...
    product_template_id = fields.Many2many('product.template')
    ...
    de_sftp_config_id = fields.Many2one('sftp.config', string='DE Server', tracking=True, required=True)
    de_target_folder = fields.Char('DE Target Folder', tracking=True)
    qde_target_folder = fields.Char('QDE Target Folder', tracking=True)
    partner_sftp_config_id = fields.Many2one('sftp.config', string='Partner Server', tracking=True)
    status_target_folder = fields.Char('Status Target Folder', tracking=True)
    active = fields.Boolean(default=True)
    status = fields.Selection(selection=[
        ("new", "New"),
        ("approve", "Approved to Launch"),
        ("running", "Running"),
        ("done", "Done"),
